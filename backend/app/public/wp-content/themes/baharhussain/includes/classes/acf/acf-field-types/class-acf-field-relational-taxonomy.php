...
class Acf_Field_Relational_Taxonomy extends \acf_field {
	...
	public function initialize() {
		...
		$this->doc_url       = function_exists( 'acf_add_url_utm_tags' ) ? acf_add_url_utm_tags( 'https://www.advancedcustomfields.com/resources/taxonomy/', 'docs', 'field-type-selection' ) : 'https://www.advancedcustomfields.com/resources/taxonomy/';
		$this->defaults      = array(
			'taxonomy'   => 'category',
			'post_type'  => 'post',
			'field_type' => 'checkbox',
	...
	public function ajax_query() {
		...
		// get choices.
		$post_data = array(
			'post_id'   => isset( $_POST['post_id'] ) ? absint( $_POST['post_id'] ) : 0, // phpcs:ignore WordPress.Security.NonceVerification.Missing -- nonce verified by acf_verify_ajax() above.
			's'         => isset( $_POST['s'] ) ? sanitize_text_field( wp_unslash( $_POST['s'] ) ) : '', // phpcs:ignore WordPress.Security.NonceVerification.Missing
			...
			'paged'     => isset( $_POST['paged'] ) ? absint( $_POST['paged'] ) : 0, // phpcs:ignore WordPress.Security.NonceVerification.Missing
		);
		$response  = $this->get_ajax_query( $post_data );
		// return.
