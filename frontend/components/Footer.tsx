  ...
  query FooterMenus {
    ...
    themeOptions {
      baharHussainTheme {
        ...
        thsVarFooterLinkedin
        thsVarFooterPhone
        thsVarFooterCtaHeading
        thsVarFooterCtaText
        thsVarFooterCtaLink {
          title
          url
...
type FooterMenusResponse = {
  ...
  themeOptions: { baharHussainTheme: FooterSettings | null } | null;
};
...
type FooterSettings = {
  ...
  thsVarFooterEmail: string | null;
  thsVarFooterLocation: string | null;
  thsVarFooterLinkedin: string | null;
  thsVarFooterPhone: string | null;
  thsVarFooterCtaHeading: string | null;
  thsVarFooterCtaText: string | null;
  thsVarFooterCtaLink: AcfLink | null;
...
function normalizeLink(url: string, wordpressOrigin: string) {
  const parsedUrl = new URL(url, wordpressOrigin);
  const external = parsedUrl.origin !== wordpressOrigin;

  return {
    external,
    href: external ? url : `${parsedUrl.pathname}${parsedUrl.search}${parsedUrl.hash}`,
  };
}

export default async function Footer() {
  const { footerMenu, legalMenu, themeOptions } = await fetchGraphQL<FooterMenusResponse>(footerMenusQuery);
  ...
  const footer = themeOptions?.baharHussainTheme;
  const projectLink = footer?.thsVarFooterCtaLink;
  const meetingLink = footer?.thsVarFooterMeetingLink;
  const projectDestination = projectLink?.url ? normalizeLink(projectLink.url, wordpressOrigin) : null;
  const meetingDestination = meetingLink?.url ? normalizeLink(meetingLink.url, wordpressOrigin) : null;
  const phoneHref = footer?.thsVarFooterPhone?.replace(/[^\d+]/g, "") ?? "";
          ...
          <div className="site-footer__box">
            <div className="site-footer__main">
              <div className="site-footer__brand">
                ...
                {footer?.thsVarFooterLogoText && <p>{footer.thsVarFooterLogoText}</p>}
              </div>
              ...
              <div className="site-footer__contact-column">
                ...
                <ul className="site-footer__contact">
                  {footer?.thsVarFooterEmail && <li>
                    <span aria-hidden="true"><svg viewBox="0 0 24 24" focusable="false"><path d="M4 6h16v12H4z" /><path d="m4 7 8 6 8-6" /></svg></span>
                    <a href={`mailto:${footer.thsVarFooterEmail}`}>{footer.thsVarFooterEmail}</a>
                  </li>}
                  {footer?.thsVarFooterLocation && <li>
                    ...
                    {footer.thsVarFooterLocation}
                  </li>}
                  {footer?.thsVarFooterLinkedin && <li>
                    ...
                    <a href={footer.thsVarFooterLinkedin} target="_blank" rel="noopener noreferrer">Connect on LinkedIn</a>
                  </li>}
                  {footer?.thsVarFooterPhone && <li>
                    ...
                    <a href={`tel:${phoneHref}`}>{footer.thsVarFooterPhone}</a>
                  </li>}
                </ul>
              </div>
              ...
              <div className="site-footer__cta">
                ...
                {footer?.thsVarFooterCtaHeading && <h2>{footer.thsVarFooterCtaHeading}</h2>}
                {footer?.thsVarFooterCtaText && <p>{footer.thsVarFooterCtaText}</p>}
                {projectLink?.title && projectDestination && (
                  projectDestination.external ? (
                    <a className="button main-btn" href={projectDestination.href} target={projectLink.target || undefined} rel={projectLink.target === "_blank" ? "noopener noreferrer" : undefined}>
                      <span className="button-text">{projectLink.title}</span>
                      <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="M5 12h14" /><path d="m13 6 6 6-6 6" /></svg>
                    </a>
                  ) : (
                    <Link className="button main-btn" href={projectDestination.href} target={projectLink.target || undefined}>
                      <span className="button-text">{projectLink.title}</span>
                      <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="M5 12h14" /><path d="m13 6 6 6-6 6" /></svg>
                    </Link>
                  )
                )}
                {meetingLink?.title && meetingDestination && (
                <a className="button outline-btn" href={meetingDestination.href} target={meetingLink.target || undefined} rel={meetingLink.target === "_blank" ? "noopener noreferrer" : undefined} title={meetingLink.title}>
                  ...
                  <span className="button-text">{meetingLink.title}</span>
                </a>
                )}
              </div>
            </div>
